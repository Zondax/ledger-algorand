...
#define CHECK_ERROR_ALGO(FUNC_CALL) do { \
    if (!FUNC_CALL) {               \
      return false;                 \
    }                               \
  } while (0)
...
//   } else if (b == MAP16) {
//     CHECK_ERROR_ALGO(next_byte(bufp, buf_end, &a));
//     CHECK_ERROR_ALGO(next_byte(bufp, buf_end, &b));
//     *map_count = (a << 8) | b;
//   } else {
...
//     str_len = b - FIXSTR_0;
//   } else if (b == STR8) {
//     CHECK_ERROR_ALGO(next_byte(bufp, buf_end, &str_len));
//   } else {
...
//   CHECK_ERROR_ALGO(decode_string(bufp, buf_end, strbuf, strbuflen-1));
...
{
  size_t map_count;
  CHECK_ERROR_ALGO(decode_fixsz(bufp, buf_end, FIXMAP_0, FIXMAP_15, &map_count));

  for (size_t i = 0; i < map_count; i++) {
    char key[32];
    CHECK_ERROR_ALGO(decode_string_nullterm(bufp, buf_end, key, sizeof(key)));

    if (!strcmp(key, "t")) {
      CHECK_ERROR_ALGO(decode_uint64(bufp, buf_end, &res->total));
    } else if (!strcmp(key, "dc")) {
      CHECK_ERROR_ALGO(decode_uint64(bufp, buf_end, &res->decimals));
    } else if (!strcmp(key, "df")) {
      CHECK_ERROR_ALGO(decode_bool(bufp, buf_end, &res->default_frozen));
    } else if (!strcmp(key, "un")) {
      CHECK_ERROR_ALGO(decode_string(bufp, buf_end, res->unitname, sizeof(res->unitname)));
    } else if (!strcmp(key, "an")) {
      CHECK_ERROR_ALGO(decode_string(bufp, buf_end, res->assetname, sizeof(res->assetname)));
    } else if (!strcmp(key, "au")) {
      CHECK_ERROR_ALGO(decode_string(bufp, buf_end, res->url, sizeof(res->url)));
    } else if (!strcmp(key, "am")) {
      CHECK_ERROR_ALGO(decode_bin_fixed(bufp, buf_end, res->metadata_hash, sizeof(res->metadata_hash)));
    } else if (!strcmp(key, "m")) {
      CHECK_ERROR_ALGO(decode_bin_fixed(bufp, buf_end, res->manager, sizeof(res->manager)));
    } else if (!strcmp(key, "r")) {
      CHECK_ERROR_ALGO(decode_bin_fixed(bufp, buf_end, res->reserve, sizeof(res->reserve)));
    } else if (!strcmp(key, "f")) {
      CHECK_ERROR_ALGO(decode_bin_fixed(bufp, buf_end, res->freeze, sizeof(res->freeze)));
    } else if (!strcmp(key, "c")) {
      CHECK_ERROR_ALGO(decode_bin_fixed(bufp, buf_end, res->clawback, sizeof(res->clawback)));
    } else {
...
decode_accounts(uint8_t **bufp, uint8_t *buf_end, uint8_t accounts[][32], size_t *num_accounts, size_t max_accounts)
{
  ...
  for (size_t i = 0; i < arr_count; i++) {
    CHECK_ERROR_ALGO(decode_bin_fixed(bufp, buf_end, accounts[i], sizeof(accounts[0])));
  }
...
decode_u64_array(uint8_t **bufp, uint8_t *buf_end, uint64_t elems[], size_t *num_elems, size_t max_elems, const char *elem_name) {
  ...
  for (size_t i = 0; i < arr_count; i++) {
    CHECK_ERROR_ALGO(decode_uint64(bufp, buf_end, &elems[i]));
  }
...
decode_state_schema(uint8_t **bufp, uint8_t *buf_end, struct state_schema *res)
{
  ...
  for (size_t i = 0; i < map_count; i++) {
    ...
    CHECK_ERROR_ALGO(decode_string_nullterm(bufp, buf_end, key, sizeof(key)));
    if (!strcmp(key, "nui")) {
      CHECK_ERROR_ALGO(decode_uint64(bufp, buf_end, &res->num_uint));
    } else if (!strcmp(key, "nbs")) {
      CHECK_ERROR_ALGO(decode_uint64(bufp, buf_end, &res->num_byteslice));
    } else {
...
static bool tx_decode_helper(uint8_t *buf, uint8_t *buf_end, parser_tx_t *t)
{
  size_t map_count;

  CHECK_ERROR_ALGO(decode_mapsz(&buf, buf_end, &map_count));

  for (size_t i = 0; i < map_count; i++) {
    char key[32];
    CHECK_ERROR_ALGO(decode_string_nullterm(&buf, buf_end, key, sizeof(key)));
    ...
    if (!strcmp(key, "type")) {
      char tbuf[16];
      CHECK_ERROR_ALGO(decode_string_nullterm(&buf, buf_end, tbuf, sizeof(tbuf)));
      ...
      if (!strcmp(tbuf, "pay")) {
        ...
        t->type = TX_ASSET_XFER;
      } else if (!strcmp(tbuf, "afrz")) {
        t->type = TX_ASSET_FREEZE;
      } else if (!strcmp(tbuf, "acfg")) {
        t->type = TX_ASSET_CONFIG;
      } else if (!strcmp(tbuf, "appl")) {
        t->type = TX_APPLICATION;
      } else {
    ...
    // Common to all transactions
      ...
      CHECK_ERROR_ALGO(decode_bin_fixed(&buf, buf_end, t->sender, sizeof(t->sender)));
    } else if (!strcmp(key, "rekey")) {
      CHECK_ERROR_ALGO(decode_bin_fixed(&buf, buf_end, t->rekey, sizeof(t->rekey)));
    } else if (!strcmp(key, "fee")) {
      CHECK_ERROR_ALGO(decode_uint64(&buf, buf_end, &t->fee));
    } else if (!strcmp(key, "fv")) {
      CHECK_ERROR_ALGO(decode_uint64(&buf, buf_end, &t->firstValid));
    } else if (!strcmp(key, "lv")) {
      CHECK_ERROR_ALGO(decode_uint64(&buf, buf_end, &t->lastValid));
    } else if (!strcmp(key, "gen")) {
      CHECK_ERROR_ALGO(decode_string(&buf, buf_end, t->genesisID, sizeof(t->genesisID)));
    } else if (!strcmp(key, "gh")) {
      CHECK_ERROR_ALGO(decode_bin_fixed(&buf, buf_end, t->genesisHash, sizeof(t->genesisHash)));
    } else if (!strcmp(key, "grp")) {
      CHECK_ERROR_ALGO(decode_bin_fixed(&buf, buf_end, t->groupID, sizeof(t->groupID)));
    } else if (!strcmp(key, "note")) {
      CHECK_ERROR_ALGO(decode_bin_var(&buf, buf_end, t->note, &t->note_len, sizeof(t->note)));

    //Payment
    } else if (!strcmp(key, "amt")) {
      CHECK_ERROR_ALGO(decode_uint64(&buf, buf_end, &t->payment.amount));
    } else if (!strcmp(key, "rcv")) {
      CHECK_ERROR_ALGO(decode_bin_fixed(&buf, buf_end, t->payment.receiver, sizeof(t->payment.receiver)));
    } else if (!strcmp(key, "close")) {
      CHECK_ERROR_ALGO(decode_bin_fixed(&buf, buf_end, t->payment.close, sizeof(t->payment.close)));

    //Keyreg
    } else if (!strcmp(key, "selkey")) {
      CHECK_ERROR_ALGO(decode_bin_fixed(&buf, buf_end, t->keyreg.vrfpk, sizeof(t->keyreg.vrfpk)));
    } else if (!strcmp(key, "sprfkey")) {
      CHECK_ERROR_ALGO(decode_bin_fixed(&buf, buf_end, t->keyreg.sprfkey, sizeof(t->keyreg.sprfkey)));
    } else if (!strcmp(key, "votekey")) {
      CHECK_ERROR_ALGO(decode_bin_fixed(&buf, buf_end, t->keyreg.votepk, sizeof(t->keyreg.votepk)));
    } else if (!strcmp(key, "votefst")) {
      CHECK_ERROR_ALGO(decode_uint64(&buf, buf_end, &t->keyreg.voteFirst));
    } else if (!strcmp(key, "votelst")) {
      CHECK_ERROR_ALGO(decode_uint64(&buf, buf_end, &t->keyreg.voteLast));
    } else if (!strcmp(key, "votekd")) {
      CHECK_ERROR_ALGO(decode_uint64(&buf, buf_end, &t->keyreg.keyDilution));
    } else if (!strcmp(key, "nonpart")) {
      CHECK_ERROR_ALGO(decode_bool(&buf, buf_end, &t->keyreg.nonpartFlag));

    // Asset XFER
    } else if (!strcmp(key, "aamt")) {
      CHECK_ERROR_ALGO(decode_uint64(&buf, buf_end, &t->asset_xfer.amount));
    } else if (!strcmp(key, "aclose")) {
      CHECK_ERROR_ALGO(decode_bin_fixed(&buf, buf_end, t->asset_xfer.close, sizeof(t->asset_xfer.close)));
    } else if (!strcmp(key, "arcv")) {
      CHECK_ERROR_ALGO(decode_bin_fixed(&buf, buf_end, t->asset_xfer.receiver, sizeof(t->asset_xfer.receiver)));
    } else if (!strcmp(key, "asnd")) {
      CHECK_ERROR_ALGO(decode_bin_fixed(&buf, buf_end, t->asset_xfer.sender, sizeof(t->asset_xfer.sender)));
    } else if (!strcmp(key, "xaid")) {
      CHECK_ERROR_ALGO(decode_uint64(&buf, buf_end, &t->asset_xfer.id));

    // Asset FREEZE
    } else if (!strcmp(key, "faid")) {
      CHECK_ERROR_ALGO(decode_uint64(&buf, buf_end, &t->asset_freeze.id));
    } else if (!strcmp(key, "fadd")) {
      CHECK_ERROR_ALGO(decode_bin_fixed(&buf, buf_end, t->asset_freeze.account, sizeof(t->asset_freeze.account)));
    } else if (!strcmp(key, "afrz")) {
      CHECK_ERROR_ALGO(decode_bool(&buf, buf_end, &t->asset_freeze.flag));

    // Asset CONFIG
    } else if (!strcmp(key, "caid")) {
      CHECK_ERROR_ALGO(decode_uint64(&buf, buf_end, &t->asset_config.id));
    } else if (!strcmp(key, "apar")) {
      CHECK_ERROR_ALGO(decode_asset_params(&buf, buf_end, &t->asset_config.params));

    //Application
    } else if (!strcmp(key, "apid")) {
      CHECK_ERROR_ALGO(decode_uint64(&buf, buf_end, &t->application.id));
    } else if (!strcmp(key, "apaa")) {
      CHECK_ERROR_ALGO(decode_app_args(&buf, buf_end, t->application.app_args, t->application.app_args_len, &t->application.num_app_args, COUNT(t->application.app_args)));
    } else if (!strcmp(key, "apap")) {
      CHECK_ERROR_ALGO(decode_bin_var(&buf, buf_end, t->application.aprog, &t->application.aprog_len, sizeof(t->application.aprog)));
    } else if (!strcmp(key, "apsu")) {
      CHECK_ERROR_ALGO(decode_bin_var(&buf, buf_end, t->application.cprog, &t->application.cprog_len, sizeof(t->application.cprog)));
    } else if (!strcmp(key, "apan")) {
      CHECK_ERROR_ALGO(decode_uint64(&buf, buf_end, &t->application.oncompletion));
    } else if (!strcmp(key, "apat")) {
      CHECK_ERROR_ALGO(decode_accounts(&buf, buf_end, t->application.accounts, &t->application.num_accounts, COUNT(t->application.accounts)));
    } else if (!strcmp(key, "apls")) {
      CHECK_ERROR_ALGO(decode_state_schema(&buf, buf_end, &t->application.local_schema));
    } else if (!strcmp(key, "apgs")) {
      CHECK_ERROR_ALGO(decode_state_schema(&buf, buf_end, &t->application.global_schema));
    } else if (!strcmp(key, "apfa")) {
      CHECK_ERROR_ALGO(decode_u64_array(&buf, buf_end, t->application.foreign_apps, &t->application.num_foreign_apps, COUNT(t->application.foreign_apps), "foreign apps"));
    } else if (!strcmp(key, "apas")) {
      CHECK_ERROR_ALGO(decode_u64_array(&buf, buf_end, t->application.foreign_assets, &t->application.num_foreign_assets, COUNT(t->application.foreign_assets), "foreign assets"));
...
char*
tx_decode(uint8_t *buf, int buflen, parser_tx_t *t)
{
